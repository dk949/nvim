...
function M.setup(conf)
    ...
    ---@type string
    local enable_list = vim.iter(conf):map(function(c) return ":TSEnable " .. c end):join(" | ")

    -- Scheduling this to avoid waiting for TS to load
    vim.schedule(function()
        vim.cmd(enable_list)
        vim.b.treesitter_disable_list = vim.iter(conf):map(function(c) return ":TSDisable " .. c end):join(" | ")
    end)
